  ...
  {messages.map((msg, index) => (
   <div key={index} className={`flex items-end gap-2 justify-end ${msg.senderId !== 'authUser?._id' && 'flex-row-reverse'}`}>
       {msg.image ? (
        ...
        <p className={`p-2 max-w-[200px] md:text-sm font-light round-lg mb-8 break-all bg-violet-500/30 text-white ${msg.senderId === 'authUser?._id' ? 'rounded-br-none' : 'rounded-bl-none'}`}>
          {msg.text}</p>
       )}
